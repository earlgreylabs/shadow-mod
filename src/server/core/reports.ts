...
/**
 * Renders a Comparison report as a Markdown message for the Observer.
...
/**
 * Renders a Comparison report framed for the Reviewer.
 * Shows how the Observer performed relative to both the Reviewer's call and the final outcome.
 */
export function formatReviewerReportMessage(report: Report): string {
  const observerMatchedReviewer = report.agreement ? '✅ Match' : '❌ Diverged';
  const observerLabel = MOD_ACTION_LABELS[report.observer.action] ?? report.observer.action;
  const reviewerLabel = MOD_ACTION_LABELS[report.reviewer.action] ?? report.reviewer.action;

  return [
    `**ShadowMod: Observer update** — [${report.postTitle}](${report.postPermalink})`,
    '',
    `u/${report.observer.observerName} completed an observation on a post you reviewed.`,
    '',
    `| | Decision | Reasoning |`,
    `|---|---|---|`,
    `| u/${report.observer.observerName} | ${observerLabel} | ${report.observer.reason} |`,
    `| Your call | ${reviewerLabel} | ${report.reviewer.reason} |`,
    `| Final outcome | ${report.finalAction} | N/A |`,
    '',
    `**Observer vs your call:** ${observerMatchedReviewer}`,
    '',
    report.agreement
      ? `u/${report.observer.observerName} matched your judgment.`
      : `u/${report.observer.observerName} chose differently from you. Consider reaching out to discuss.`,
    '',
    `_Generated by ShadowMod at ${new Date(report.generatedAt).toUTCString()}_`,
  ].join('\n');
}
...
export async function generateReport(data: ReportJobData, subredditName: string): Promise<void> {
  ...
  const observerMessage = formatReportMessage(report);
  const reviewerMessage = formatReviewerReportMessage(report);
